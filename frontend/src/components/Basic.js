...
function Basic() {
    ...
    const [custName, setCustName] = useState(''); // Track the user's name
    useEffect(() => {
    ...
    const handleSubmit = (evt) => {
        evt.preventDefault();
        const request_temp = { sender: "user", sender_id: custName, msg: inputMessage };
    ...
    const rasaAPI = async (msg) => {
        await fetch('http://localhost:5005/webhooks/rest/webhook', {
            ...
            body: JSON.stringify({ "sender": custName || "user", "message": msg }),
        })
                ...
                if (response && response.length > 0) {
                    ...
                    response.forEach((temp) => {
                        ...
                        if (recipient_msg) {
                            ...
                            console.error("One of the responses does not contain 'text' property.");
                        }
            
                        // Optionally, handle recipient_id if necessary
                        if (temp["recipient_id"] && !custName) {
                            setCustName(temp["recipient_id"]);
                        }
                    });
                } else {
            ...
            .catch((error) => {
                console.error("Error in fetching data from Rasa API: ", error);
                setBotTyping(false);
            });
